...
4. Dans chacun de vos fichiers. csproj, mettez à jour la version `netcoreapp3.1` cible de `netstandard2.1` .NET Framework vers (ou s’il s’agit d’un projet de bibliothèque).
...
6. Sélectionnez **Outils** -> **ligne** -> de commande**invite de commandes développeur**
...
3. Dans chacun de vos fichiers. csproj, mettez à jour la version `netcoreapp3.1` cible de `netstandard2.1` .NET Framework vers (ou s’il s’agit d’un projet de bibliothèque).
    Autrement dit, modifiez les lignes du formulaire :
...
1. Mettre à `iqsharp` jour le noyau 
...
2. Vérifier la `iqsharp` version
    ...
    Ne vous inquiétez `iqsharp` pas si votre version est plus élevée, elle doit correspondre à la [version la plus récente](xref:microsoft.quantum.relnotes).

3. Mettre à `qsharp` jour le package
...
5. Exécutez la commande suivante à partir de l’emplacement `.qs` de vos fichiers
...
### <a name="c-using-the-dotnet-command-line-tool"></a>C#, à l' `dotnet` aide de l’outil en ligne de commande
