---
title: 'Q # bibliothèques standard-caractérisation | Microsoft Docs'
description: 'Q # bibliothèques standard-caractérisation'
...
Cet obscurcissement de l’État Quantum de l’utilisateur est reflété dans le fait que Q # n’expose pas ou *même ne définit pas l’état des* programmes quantiques.
...
Le qubit auxiliaire utilisé comme contrôle pour `U(m)` est ensuite mesuré dans la base $X $ pour obtenir un `Result`classique unique.
...
Il s’agit d’une instruction plus faible que dans le cas discret, puisque nous pouvons construire une <xref:microsoft.quantum.oracles.discreteoracle> en restreignant $t = m\,\delta t $ pour quelques-\delta t $ fixe.
...
Par exemple, l’opération de <xref:microsoft.quantum.characterization.robustphaseestimation> décrite dans la section suivante a la signature suivante :

```qsharp
operation RobustPhaseEstimation(bitsPrecision : Int, oracle : DiscreteOracle, eigenstate : Qubit[])  : Double
```

L’entrée `bitsPrecision` est propre à `RobustPhaseEstimation`, tandis que les `oracle` et les `eigenstate` sont en commun.
