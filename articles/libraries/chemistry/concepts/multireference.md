...
---
# <a name="correlated-wavefunctions"></a>Wavefunctions corrélé
...
Un État à plusieurs références $ \ket{\psi_{\rm {MCSCF}}} $ peut être spécifié explicitement comme une combinaison linéaire de $N determininants $-Electron.
...
Il est également possible de spécifier une wavefunction d’unités $ \ket{\psi_{\rm {UCC}}} $ couplée à l’aide de la bibliothèque chemistery. Dans ce cas, nous avons un état de référence unique, par exemple, $ \ket{\psi_{\rm{SCF}}} $. Les composants des wavefunction de clusters à couplage unitaire sont ensuite spécifiés implicitement via un opérateur unitaire agissant sur un état de référence.
...
et l’opérateur de cluster à deux corps (double) se présente sous la forme \begin{align} T_2 = \sum_{PQRS}t ^ {PQ} _ {RS} a ^ \dagger_p a ^ \dagger_q a_r A_S.
\end{align}
...
Par exemple, laissez $ \ket{\psi_{\rm{SCF}}} = a ^ \dagger_1 a ^ \dagger_2\ket{0}$ et laissez $T = 0,123 a ^ \dagger_0 A_1 + 0,456 a ^ \dagger_0a ^ \dagger_3 A_1 a_2-0,789 a ^ \dagger_3a ^ \dagger_2 A_1 a_0 $. Ensuite, cet État est instancié dans la bibliothèque chimie comme suit.
```csharp
...
La sélection de spin convervation peut être rendue explicite en spécifiant `SpinOrbital` index à la place des index d’entiers. Par exemple, laissez $ \ket{\psi_{\rm{SCF}}} = a ^ \dagger_{1, \uparrow} a ^ \dagger_{2, \downarrow}\ket{0}$ et laissez $T = 0,123 a ^ \dagger_{0, \uparrow} a_ {1, \uparrow} + 0,456 a ^ \dagger_{0, \uparrow} a ^ \dagger_{3, \downarrow} a_ {1, \uparrow} a_ {2, \ downarrow}-0,789 a ^ \dagger_{3, \uparrow} a ^ \dagger_{2, \uparrow} a_ {1, \uparrow} a_ {0, \uparrow} $ est spin convserving. Ensuite, cet État est instancié dans la bibliothèque chimie comme suit.
```csharp
// Create a list of indices of the creation operators
// for the single-reference state
