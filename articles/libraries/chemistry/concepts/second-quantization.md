...
Au lieu d’affecter chaque $N _e $ Electron à un état spécifique (ou orbital), une deuxième quantification effectue le suivi de chaque orbital et stocke si un électron est présent dans chacun d’eux et, en même temps, garantit automatiquement les propriétés de symétrie du fonction Wave correspondante.
...
En guise d’exemple de deuxième quantification en action, supposons que $ \psi_0\cdots \psi_{N-1} $ est un ensemble orthonormal d’orbites spatiales.
...
Cela signifie que nous pouvons penser formellement à la base pour la partie de rotation et spatiale de la fonction Wave en tant que $ \psi_{0} \cdots \psi_{2N-1} $ où chacun des index est maintenant une énumération d’un $ (j, \sigma) $.
...
Par exemple, \begin{align} a ^ \dagger_1 \ket{0}_ 1 = \ket{1}_ 1, \quad a ^ \dagger_1 \ket{1}1-0, \quad A_1 \ket{0}_ 1 = 0, \quad A_1 \ket{1}_ 1 = \ket{0}_ 1.
\end{align} Notez que ici $a ^ \dagger_1 \ket{1}1 à 0 $ et $a _ 1 \ket{0}_ 1 $ génère le vecteur zéro non $ \ket{0}_ 1 $.
...
Pour un système de $k $ fermions, dans la deuxième quantification, l’action de l’opérateur de création $a ^ \dagger_i $ est donnée par $ $ a ^ \dagger_i \ket{n_1, N_2, \ldots, 0_i, \ldots, n_k} = (-1) ^ {S_i} \ket{n_1, N_2, \ldots, 1_i , \ldots, n_k}, $ $ et $ $ a ^ \dagger_i \ket{n_1, N_2, \ldots, 1_i, \ldots, n_k} = 0, $ $ où $S _i = \sum_{j < i} a ^ \dagger_j a_j $ mesure le nombre total de fermions qui sont dans l’état d’une seule particule et qui ont un index $j < i $.
...
Cela signifie que $ $ a ^ \dagger_2 a ^ \dagger_1 \ket{0} =-a ^ \dagger_1 a ^ \dagger_2 \ket{0}.
Les opérateurs $ $ sont dits « anti-muet » et, en général, pour tout $i, j $ nous \begin{align} un ^ \dagger_i a ^ \dagger_j =-(1-\delta_{i, j}) ^ \dagger_j a ^ \dagger_i, \quad a ^ \dagger_i a_j = \delta_{i, j}-a_j a ^ \dagger_i.
...
Par exemple, considérez la $a de la _0 ^ \dagger A_1 ^ \dagger A_1 a_0 =-A_1 ^ \dagger a_0 ^ \dagger A_1 a_0 $.
...
\begin{align} h\_{PQRS} & = \int\_{-\infty} ^ \infty \int\_{-\infty} ^ \infty\psi\_p ^\*(x\_1) \Psi\_q ^\*(x\_2) \left (\frac{1}{| X_1-X_2 |} \ Right) \Psi\_r (x\_2) \Psi\_s (x\_1) \mathrm{d} ^ 3x\_1 \ mathrm {d} ^ 3x\_2. \ label {EQ : intégrales} \end{align}

Les termes $h\_{PQ} $ sont considérés comme des intégrales à un seul électron, car tous ces termes impliquent uniquement des électrons uniques et de même $h\_{PQRS} $ sont les intégrales à deux électrons.
...
Par exemple, $h _ {PQ} a ^ \dagger_p a_q $ décrit les sauts d’électrons de spin orbital $q $ to spin orbital $p $.
De même, le terme $h _ {PQRS} a ^ \dagger_p a ^ \dagger_q a_r A_S $ (pour distinct $p, q, r, s $) décrit deux électrons dans des orbites de spin $r $ et $s $ Dispersing et se terminant par des orbites de spins $p $ et $q $.
Si $r = q $ et $p = s $ Then $h _ {PRRP} a ^ \dagger_p a ^ \dagger_r a_r a_p = H_ {PRRP} n_P n_r $ donne la pénalité d’énergie associée aux deux électrons qui sont proches les uns des autres, mais ne décrit pas un processus dynamique.
...
Par exemple, considérez la $H Hamiltony = a_0 ^ \dagger A_1 ^ \dagger A_1 a_0-A_1 ^ \dagger a_0 ^ \dagger A_1 a_0 = 2a_0 ^ \dagger A_1 ^ \dagger A_1 a_0 $, qui est une somme des termes construits ci-dessus.
...
Par exemple, dans le cas où $p, q, r, s $ sont des index distincts, nous pouvons utiliser les règles anti-commutants pour montrer que : $ $ a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} =-a ^ \dagger\_{q} a ^ \dagger\_{ p} a\_{r} a\_{s} =-a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{s} a\_{r} = a ^ \dagger\_{q} a ^ \dagger\_{p} a\_{r} a\_{r}.
$$

En outre, comme $H $ est Hermitian, chaque opérateur fermionic non-Hermitian, par exemple $h\_{PQRS} a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} $, a un conjugué Hermitian qui se trouve également dans $H $. Afin d’indexer de manière unique des groupes de termes caractérisés par ces Symmetries, nous définissons un classement canonique sur les index $ (i\_1, \cdots, i\_n, j\_1, \cdots, j\_m) $ de toute séquence d’opérateurs $n + m $ fermionic $a ^ \dagger\_{i\_1} \cdots un ^ \dagger\_{i\_n} a\_{j\_1} \cdots a\_{j\_m} $as suit :
