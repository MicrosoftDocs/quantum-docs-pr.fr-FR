...
La matrice $u $ est ensuite optimisée pour réduire l’énergie attendue $ \bra {0} \ prod_ {j = 0} ^ {n-1} \widetilde{a} \_ j H \prod \_ {k = 0} ^ {n-1} \widetilde{a} ^ \ dagger_k \ket {0} $. Ces problèmes d’optimisation peuvent être génériques, en pratique, l’algorithme Hartree – Fock tend à converger rapidement vers une solution presque optimale pour le problème d’optimisation, en particulier pour les molécules de Shell fermé dans les géométries d’équilibre. Nous pouvons spécifier ces États en tant qu’instance de l' `FermionWavefunction` objet. Par exemple, l’État $a ^ \ dagger_ {1} un ^ \ dagger_ {2} un ^ \ dagger_ {6} \ket {0} $ est instancié dans la bibliothèque chimie comme suit.
```csharp
