...
Une fois que nous avons construit l’opérateur de la construction de la même façon en traduisant le courant de la Hamilton classique en langage quantique, nous pouvons exprimer la dynamique d’une quantité mécanique arbitraire (par exemple, un opérateur de Quantum mécanique) $ \hat{f} (t) $ via \begin{align} \frac{d}{DT} \hat{f} = \ partial_t \hat{f} + [\hat{f}, \hat{H}], \end{align} où $ [f, H] = fH-HF $ est appelé commutator.
