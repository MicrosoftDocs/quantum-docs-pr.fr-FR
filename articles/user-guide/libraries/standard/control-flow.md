...
<span data-ttu-id="49e21-107">Nous pouvons exprimer ce modèle à l’aide d’itérations et de `for` boucles :</span><span class="sxs-lookup"><span data-stu-id="49e21-107">We can express this pattern by using iteration and `for` loops:</span></span>
...
<span data-ttu-id="49e21-108">Exprimée en termes de <xref:Microsoft.Quantum.Canon.ApplyToEachCA> et de fonctions de manipulation de tableau telles que <xref:Microsoft.Quantum.Arrays.Zipped> , toutefois, cette opération est beaucoup plus rapide et plus facile à lire :</span><span class="sxs-lookup"><span data-stu-id="49e21-108">Expressed in terms of <xref:Microsoft.Quantum.Canon.ApplyToEachCA> and array manipulation functions such as <xref:Microsoft.Quantum.Arrays.Zipped>, however, this is much shorter and easier to read:</span></span>

```qsharp
ApplyToEachCA(CNOT, Zip(register[0..nQubits - 2], register[1..nQubits - 1]));
```

<span data-ttu-id="49e21-109">Dans le reste de cette section, nous vous fournirons un certain nombre d’exemples d’utilisation des diverses opérations et fonctions de contrôle de workflow fournies par Canon pour exprimer les programmes Quantum de manière compacte.</span><span class="sxs-lookup"><span data-stu-id="49e21-109">In the rest of this section, we will provide a number of examples of how to use the various flow control operations and functions provided by the canon to compactly express quantum programs.</span></span>

## <a name="applying-operations-and-functions-over-arrays-and-ranges"></a><span data-ttu-id="49e21-110">Application d’opérations et de fonctions sur des tableaux et des plages</span><span class="sxs-lookup"><span data-stu-id="49e21-110">Applying Operations and Functions over Arrays and Ranges</span></span> ##
...
<span data-ttu-id="49e21-114">Nous pouvons ensuite utiliser cette nouvelle opération `HAll(register)` pour appliquer $H \Otimes h \otimes \cdots \Otimes h $.</span><span class="sxs-lookup"><span data-stu-id="49e21-114">We can then use this new operation as `HAll(register)` to apply $H \otimes H \otimes \cdots \otimes H$.</span></span>

<span data-ttu-id="49e21-115">Cette opération est lourde, cependant, en particulier dans un algorithme plus grand.</span><span class="sxs-lookup"><span data-stu-id="49e21-115">This is cumbersome to do, however, especially in a larger algorithm.</span></span>
<span data-ttu-id="49e21-116">En outre, cette approche est spécialisée pour l’opération particulière que nous souhaitons appliquer à chaque qubit.</span><span class="sxs-lookup"><span data-stu-id="49e21-116">Moreover, this approach is specialized to the particular operation that we wish to apply to each qubit.</span></span>
<span data-ttu-id="49e21-117">Nous pouvons utiliser le fait que les opérations sont de première classe pour exprimer ce concept algorithmique plus explicitement :</span><span class="sxs-lookup"><span data-stu-id="49e21-117">We can use the fact that operations are first-class to express this algorithmic concept more explicitly:</span></span>

```qsharp
ApplyToEachCA(H, register);
```

<span data-ttu-id="49e21-118">Ici, le suffixe `CA` indique que l’appel à `ApplyToEach` est lui-même adjointable et contrôlable.</span><span class="sxs-lookup"><span data-stu-id="49e21-118">Here, the suffix `CA` indicates that the call to `ApplyToEach` is itself adjointable and controllable.</span></span>
<span data-ttu-id="49e21-119">Ainsi, si `U` prend en charge `Adjoint` et `Controlled` , les lignes suivantes sont équivalentes :</span><span class="sxs-lookup"><span data-stu-id="49e21-119">Thus, if `U` supports `Adjoint` and `Controlled`, the following lines are equivalent:</span></span>
...
<span data-ttu-id="49e21-120">En particulier, cela signifie que les appels à `ApplyToEachCA` peuvent apparaître dans les opérations pour lesquelles une spécialisation voisine est générée automatiquement.</span><span class="sxs-lookup"><span data-stu-id="49e21-120">In particular, this means that calls to `ApplyToEachCA` can appear in operations for which an adjoint specialization is auto-generated.</span></span>
<span data-ttu-id="49e21-121">De même, <xref:Microsoft.Quantum.Canon.ApplyToEachIndex> est utile pour représenter les modèles du formulaire `U(0, targets[0]); U(1, targets[1]); ...` et offre des versions pour chaque combinaison d’functors prise en charge par son entrée.</span><span class="sxs-lookup"><span data-stu-id="49e21-121">Similarly, <xref:Microsoft.Quantum.Canon.ApplyToEachIndex> is useful for representing patterns of the form `U(0, targets[0]); U(1, targets[1]); ...`, and offers versions for each combination of functors supported by its input.</span></span>

> [!TIP]
> <span data-ttu-id="49e21-122">`ApplyToEach` est paramétrable de type de sorte qu’il puisse être utilisé avec des opérations qui prennent des entrées autres que `Qubit` .</span><span class="sxs-lookup"><span data-stu-id="49e21-122">`ApplyToEach` is type-parameterized such that it can be used with operations that take inputs other than `Qubit`.</span></span>
> <span data-ttu-id="49e21-123">Par exemple, supposons qu' `codeBlocks` il s’agit d’un tableau de <xref:Microsoft.Quantum.ErrorCorrection.LogicalRegister> valeurs qui doivent être récupérées.</span><span class="sxs-lookup"><span data-stu-id="49e21-123">For example, suppose that `codeBlocks` is an array of <xref:Microsoft.Quantum.ErrorCorrection.LogicalRegister> values that need to be recovered.</span></span>
...
<span data-ttu-id="49e21-131">Les constructions de workflow de contrôle offertes par Canon prennent les opérations et fonctionnent comme entrées, de sorte qu’il est utile de pouvoir composer plusieurs opérations ou fonctions en un seul appelable.</span><span class="sxs-lookup"><span data-stu-id="49e21-131">The control flow constructs offered by the canon take operations and functions as their inputs, such that it is helpful to be able to compose several operations or functions into a single callable.</span></span>
<span data-ttu-id="49e21-132">Par exemple, le modèle $UVU ^ {\dagger} $ est extrêmement courant dans la programmation quantique, de sorte que Canon fournit l’opération <xref:Microsoft.Quantum.Canon.ApplyWith> en tant qu’abstraction pour ce modèle.</span><span class="sxs-lookup"><span data-stu-id="49e21-132">For instance, the pattern $UVU^{\dagger}$ is extremely common in quantum programming, such that the canon provides the operation <xref:Microsoft.Quantum.Canon.ApplyWith> as an abstraction for this pattern.</span></span>
<span data-ttu-id="49e21-133">Cette abstraction permet également une meilleure conformité dans les circuits, car il `Controlled` `U(qubit); V(qubit); Adjoint U(qubit);` n’est pas nécessaire d’agir sur chaque séquence `U` .</span><span class="sxs-lookup"><span data-stu-id="49e21-133">This abstraction also allows for more efficient compliation into circuits, as `Controlled` acting on the sequence `U(qubit); V(qubit); Adjoint U(qubit);` does not need to act on each `U`.</span></span>
<span data-ttu-id="49e21-134">Pour le voir, $c (U) $ est l’unité représentant `Controlled U([control], target)` et laisser $c (V) $ être défini de la même façon.</span><span class="sxs-lookup"><span data-stu-id="49e21-134">To see this, let $c(U)$ be the unitary representing `Controlled U([control], target)` and let $c(V)$ be defined in the same way.</span></span>
<span data-ttu-id="49e21-135">Ensuite, pour un état arbitraire $ \ket{\Psi} $, \begin{align} c (U) c (V) c (U) ^ \dagger \ket {1} \otimes \ket{\psi} & = \ket {1} \OTIMES (UVU ^ {\dagger} \ket{\psi}) \\ \\ & = (\boldone \otimes u) (c (V)) (\boldone \otimes U ^ \dagger) \ket {1} \otimes \ket{\Psi}.</span><span class="sxs-lookup"><span data-stu-id="49e21-135">Then for an arbitrary state $\ket{\psi}$, \begin{align} c(U) c(V) c(U)^\dagger \ket{1} \otimes \ket{\psi} & = \ket{1} \otimes (UVU^{\dagger} \ket{\psi}) \\\\ & = (\boldone \otimes U) (c(V)) (\boldone \otimes U^\dagger) \ket{1} \otimes \ket{\psi}.</span></span>
<span data-ttu-id="49e21-136">\end{align} par la définition de `Controlled` .</span><span class="sxs-lookup"><span data-stu-id="49e21-136">\end{align} by the definition of `Controlled`.</span></span>
<span data-ttu-id="49e21-137">En revanche, \begin{align} c (U) c (V) c (U) ^ \dagger \ket {0} \otimes \ket{\psi} & = \ket {0} \otimes \ket{\Psi} \\ \\ & = \ket {0} \otimes (UU ^ \dagger \ket{\Psi}) \\ \\ & = (\Boldone \otimes u) (c (V)) (\boldone \otimes U ^ \dagger) \ket {0} \otimes \ket{\Psi}.</span><span class="sxs-lookup"><span data-stu-id="49e21-137">On the other hand, \begin{align} c(U) c(V) c(U)^\dagger \ket{0} \otimes \ket{\psi} & = \ket{0} \otimes \ket{\psi} \\\\ & = \ket{0} \otimes (UU^\dagger \ket{\psi}) \\\\ & = (\boldone \otimes U) (c(V)) (\boldone \otimes U^\dagger) \ket{0} \otimes \ket{\psi}.</span></span>
<span data-ttu-id="49e21-138">\end{align} par linéarité, nous pouvons conclure que nous pouvons factoriser $U $ Out de cette manière pour tous les États d’entrée.</span><span class="sxs-lookup"><span data-stu-id="49e21-138">\end{align} By linearity, we can conclude that we can factor $U$ out in this way for all input states.</span></span>
<span data-ttu-id="49e21-139">Autrement dit, $c (UVU ^ \dagger) = U c (V) U ^ \dagger $.</span><span class="sxs-lookup"><span data-stu-id="49e21-139">That is, $c(UVU^\dagger) = U c(V) U^\dagger$.</span></span>
<span data-ttu-id="49e21-140">Étant donné que les opérations de contrôle peuvent être coûteuses en général, l’utilisation de variantes contrôlées telles que `WithC` et `WithCA` peut aider à réduire le nombre d’functors de contrôle qui doivent être appliqués.</span><span class="sxs-lookup"><span data-stu-id="49e21-140">Since controlling operations can be expensive in general, using controlled variants such as `WithC` and `WithCA` can help reduce the number of control functors that need to be applied.</span></span>

> [!NOTE]
> <span data-ttu-id="49e21-141">Une autre conséquence de la factorisation $U $ est que nous n’avons même pas besoin de savoir comment appliquer le `Controlled` functor à `U` .</span><span class="sxs-lookup"><span data-stu-id="49e21-141">One other consequence of factoring out $U$ is that we need not even know how to apply the `Controlled` functor to `U`.</span></span>
> <span data-ttu-id="49e21-142">`ApplyWithCA` par conséquent, la signature est plus faible que celle qui est attendue :</span><span class="sxs-lookup"><span data-stu-id="49e21-142">`ApplyWithCA` therefore has a weaker signature than might be expected:</span></span>
> ```qsharp
> ApplyWithCA<'T> : (('T => Unit is Adj),
>     ('T => Unit is Adj + Ctl), 'T) => Unit
> ```

<span data-ttu-id="49e21-143">De même, <xref:Microsoft.Quantum.Canon.Bound> produit des opérations qui appliquent une séquence d’autres opérations à leur tour.</span><span class="sxs-lookup"><span data-stu-id="49e21-143">Similarly, <xref:Microsoft.Quantum.Canon.Bound> produces operations which apply a sequence of other operations in turn.</span></span>
<span data-ttu-id="49e21-144">Par exemple, les éléments suivants sont équivalents :</span><span class="sxs-lookup"><span data-stu-id="49e21-144">For instance, the following are equivalent:</span></span>
...
<span data-ttu-id="49e21-145">La combinaison avec les modèles d’itération peut s’avérer particulièrement utile :</span><span class="sxs-lookup"><span data-stu-id="49e21-145">Combining with iteration patterns can make this especially useful:</span></span>
...
<span data-ttu-id="49e21-156">À ce stade, nous pouvons maintenant expliquer l’expansion Trotter – Suzuki *sans référence à la mécanique des quantums* .</span><span class="sxs-lookup"><span data-stu-id="49e21-156">At this point, we can now reason about the Trotter–Suzuki expansion *without reference to quantum mechanics at all* .</span></span>
<span data-ttu-id="49e21-157">L’expansion est effectivement un modèle d’itération très particulier motivé par $ \eqref{EQ : Trotter-Suzuki-0} $.</span><span class="sxs-lookup"><span data-stu-id="49e21-157">The expansion is effectively a very particular iteration pattern motivated by $\eqref{eq:trotter-suzuki-0}$.</span></span>
<span data-ttu-id="49e21-158">Ce modèle d’itération est implémenté par <xref:Microsoft.Quantum.Canon.DecomposedIntoTimestepsCA> :</span><span class="sxs-lookup"><span data-stu-id="49e21-158">This iteration pattern is implemented by <xref:Microsoft.Quantum.Canon.DecomposedIntoTimestepsCA>:</span></span>
...
<span data-ttu-id="49e21-161">Enfin, Canon s’appuie sur `Controlled` functor en fournissant des méthodes supplémentaires pour la condition des opérations quantiques.</span><span class="sxs-lookup"><span data-stu-id="49e21-161">Finally, the canon builds on the `Controlled` functor by providing additional ways to condition quantum operations.</span></span>
<span data-ttu-id="49e21-162">Il est courant, surtout dans le cas de l’arithmétique quantique, de conditionner les opérations sur les États de base de calcul autres que $ \ket{0\cdots 0} $.</span><span class="sxs-lookup"><span data-stu-id="49e21-162">It is common, especially in quantum arithmetic, to condition operations on computational basis states other than $\ket{0\cdots 0}$.</span></span>
<span data-ttu-id="49e21-163">À l’aide des opérations de contrôle et des fonctions présentées ci-dessus, nous pouvons obtenir des conditions de Quantum plus générales dans une instruction unique.</span><span class="sxs-lookup"><span data-stu-id="49e21-163">Using the control operations and functions introduced above, we can more general quantum conditions in a single statement.</span></span>
<span data-ttu-id="49e21-164">Passons à la procédure <xref:Microsoft.Quantum.Canon.ControlledOnBitString> (paramètres de type sans réseau), puis nous décomposons les éléments un par un.</span><span class="sxs-lookup"><span data-stu-id="49e21-164">Let's jump in to how <xref:Microsoft.Quantum.Canon.ControlledOnBitString> does it (sans type parameters), then we'll break down the pieces one by one.</span></span>
<span data-ttu-id="49e21-165">La première chose à faire est de définir une opération qui fait réellement le gros du levage de l’implémentation du contrôle sur des États de base de calcul arbitraires.</span><span class="sxs-lookup"><span data-stu-id="49e21-165">The first thing we'll need to do is to define an operation which actually does the heavy lifting of implementing the control on arbitrary computational basis states.</span></span>
<span data-ttu-id="49e21-166">Nous n’appellerons pas cette opération directement, mais nous ajoutons donc `_` au début du nom pour indiquer qu’il s’agit d’une implémentation d’une autre construction ailleurs.</span><span class="sxs-lookup"><span data-stu-id="49e21-166">We won't call this operation directly, however, and so we add `_` to the beginning of the name to indicate that it's an implementation of another construct elsewhere.</span></span>
...
<span data-ttu-id="49e21-167">Notez que nous prenons une chaîne de bits, représentée sous la forme d’un `Bool` tableau, que nous utilisons pour spécifier le conditionnement que nous souhaitons appliquer à l’opération `oracle` que nous avons donnée.</span><span class="sxs-lookup"><span data-stu-id="49e21-167">Note that we take a string of bits, represented as a `Bool` array, that we use to specify the conditioning that we want to apply to the operation `oracle` that we are given.</span></span>
<span data-ttu-id="49e21-168">Étant donné que cette opération fait directement l’application, nous devons également prendre les registres de contrôle et cibles, représentés ici sous la forme `Qubit[]` .</span><span class="sxs-lookup"><span data-stu-id="49e21-168">Since this operation actually does the application directly, we also need to take the control and target registers, both represented here as `Qubit[]`.</span></span>

<span data-ttu-id="49e21-169">Ensuite, nous constatons que le contrôle de l’état de la base de calcul $ \ket{\vec{s}} = \ket{s \_ 0 s \_ 1 \dots s \_ {n-1}} $ pour une chaîne de bits $ \vec{s} $ peut être réalisé en transformant $ \ket{\vec{s}} $ en $ \ket{0\cdots 0} $.</span><span class="sxs-lookup"><span data-stu-id="49e21-169">Next, we note that controlling on the computational basis state $\ket{\vec{s}} = \ket{s\_0 s\_1 \dots s\_{n - 1}}$ for a bit string $\vec{s}$ can be realized by transforming $\ket{\vec{s}}$ into $\ket{0\cdots 0}$.</span></span>
<span data-ttu-id="49e21-170">En particulier, $ \ket{\vec{s}} = X ^ {s \_ 0} \Otimes X ^ {s \_ 1} \otimes \Cdots \otimes x ^ {s \_ {n-1}} \ket{0\cdots 0} $.</span><span class="sxs-lookup"><span data-stu-id="49e21-170">In particular, $\ket{\vec{s}} = X^{s\_0} \otimes X^{s\_1} \otimes \cdots \otimes X^{s\_{n - 1}} \ket{0\cdots 0}$.</span></span>
<span data-ttu-id="49e21-171">Depuis $X ^ {\dagger} = X $, cela implique que $ \ket{0\dots 0} = X ^ {s \_ 0} \Otimes x ^ {s \_ 1} \otimes \Cdots \otimes X ^ {s \_ {n-1}} \ket{\vec{s}} $.</span><span class="sxs-lookup"><span data-stu-id="49e21-171">Since $X^{\dagger} = X$, this implies that $\ket{0\dots 0} = X^{s\_0} \otimes X^{s\_1} \otimes \cdots \otimes X^{s\_{n - 1}} \ket{\vec{s}}$.</span></span>
<span data-ttu-id="49e21-172">Par conséquent, nous pouvons appliquer $P = X ^ {s \_ 0} \Otimes X ^ {s \_ 1} \otimes \Cdots \otimes x ^ {s \_ {n-1}} $, appliquer `Controlled oracle` et transformer en $ \vec{s} $.</span><span class="sxs-lookup"><span data-stu-id="49e21-172">Thus, we can apply $P = X^{s\_0} \otimes X^{s\_1} \otimes \cdots \otimes X^{s\_{n - 1}}$, apply `Controlled oracle`, and transform back to $\vec{s}$.</span></span>
<span data-ttu-id="49e21-173">Cette construction est précise `ApplyWith` , donc nous écrivons le corps de notre nouvelle opération en conséquence :</span><span class="sxs-lookup"><span data-stu-id="49e21-173">This construction is precisely `ApplyWith`, so we write the body of our new operation accordingly:</span></span>
...
<span data-ttu-id="49e21-174">Ici, nous avons utilisé <xref:Microsoft.Quantum.Canon.ApplyPauliFromBitString> pour appliquer $P $, partiellement appliqué sur sa cible pour une utilisation avec `ApplyWith` .</span><span class="sxs-lookup"><span data-stu-id="49e21-174">Here, we have used <xref:Microsoft.Quantum.Canon.ApplyPauliFromBitString> to apply $P$, partially applying over its target for use with `ApplyWith`.</span></span>
<span data-ttu-id="49e21-175">Notez, toutefois, que nous devons transformer le registre de *contrôle* au format souhaité, donc nous appliquons partiellement l’opération interne `(Controlled oracle)` sur la *cible* .</span><span class="sxs-lookup"><span data-stu-id="49e21-175">Note, however, that we need to transform the *control* register to our desired form, so we partially apply the inner operation `(Controlled oracle)` on the *target* .</span></span>
<span data-ttu-id="49e21-176">Cela permet de mettre en place `ApplyWith` le registre de contrôle avec $P $, exactement comme nous le souhaitons.</span><span class="sxs-lookup"><span data-stu-id="49e21-176">This leaves `ApplyWith` acting to bracket the control register with $P$, exactly as we desired.</span></span>
