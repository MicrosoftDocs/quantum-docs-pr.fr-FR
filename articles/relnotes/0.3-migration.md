...
Un exemple d’une telle modification est le tuple de symboles dans les spécialisations définies par l’utilisateur. Dans les versions précédentes, une déclaration de spécialisation contrôlée ou contrôlée-contigut spécifiait un argument de symbole unique. à partir de 0,3, il est recommandé de fournir un tel tuple de symboles pour toute la spécialisation avec `...` indiquant que le compilateur doit migrer les symboles à partir de la déclaration pouvant être appelée, si nécessaire. 
