...
Bien que Q# ait été principalement créé en tant que langage de programmation de haut niveau pour les programmes quantiques à grande échelle, il peut tout aussi facilement être utilisé pour explorer le niveau inférieur de programmes Quantum : en traitant directement des qubits spécifiques.
...
Dans notre cas, nous allons définir une Q# opération pour effectuer la transformation de Fourier quantique qubit complète, qui présente la représentation suivante sous la forme d’un circuit :
...
> * Appeler Q# des opérations directement à partir de l’invite de commandes ou à l’aide d’un programme hôte classique
...
1. Le programme lui-même, qui est implémenté à l’aide du Q# langage de programmation Quantum, puis appelé pour s’exécuter sur un ordinateur Quantum ou un simulateur Quantum. Il s’agit de 
    - Q# opérations : sous-routines agissant sur des registres quantiques et 
    - Q# Functions : sous-routines classiques utilisées dans l’algorithme Quantum.
2. Point d’entrée utilisé pour appeler le programme Quantum et spécifier l’ordinateur cible sur lequel il doit être exécuté.
    Cela peut être effectué directement à partir de l’invite de commandes, ou par le biais d’un programme hôte écrit dans un langage de programmation classique comme Python ou C#.
...
La première partie de ce didacticiel consiste à définir l' Q# opération `Perform3qubitQFT` , qui exécute la transformation de Fourier quantique sur trois qubits. 
...
La première étape consiste à créer votre Q# projet et votre fichier.
...
### <a name="namespaces-to-access-other-no-locq-operations"></a>Espaces de noms pour accéder à d’autres Q# opérations
Dans le fichier, nous commençons par définir l’espace de noms `NamespaceQFT` qui sera accessible par le compilateur.
Pour que notre opération utilise des opérations existantes Q# , nous allons ouvrir les `Microsoft.Quantum.<>` espaces de noms appropriés.
...
Dans Q# le cadre de notre opération, nous allouez tout d’abord un registre de trois qubits à l’aide de l' `using` instruction :
...
Au cours d’une Q# opération, les instructions qui appellent callables sont évidemment exécutées dans un ordre séquentiel.
...
Nous avons donc fini d’écrire les opérations de niveau qubit de la transformation de Fourier quantique dans notre Q# opération :

<img src="../media/qft_full.PNG" alt="Three qubit quantum Fourier transform circuit diagram" width="600">

Toutefois, nous ne pouvons pas l’appeler une journée pour l’instant.
Nos qubits étaient dans l’État $ \ket {0} $ lorsque nous les avons alloués, et comme dans Q# la vie, nous devrions garder des choses de la même façon que nous les avons trouvées (ou mieux !).
...
Exiger que toutes les qubits désallouées soient explicitement définies sur $ \ket {0} $ est une fonctionnalité de base de Q# , car elle permet à d’autres opérations de connaître précisément leur état lorsqu’elles commencent à utiliser ces mêmes qubits (une ressource rare).
...
Votre Q# fichier complet doit maintenant ressembler à ceci :
...
Tandis que le Q# programme est omniprésent dans les environnements utilisés pour l’appeler, la manière dont cela variera bien évidemment. Par conséquent, suivez simplement les instructions de l’onglet correspondant à votre installation : utilisation de l' Q# application ou utilisation d’un programme hôte en Python ou C#.

#### <a name="command-prompt"></a>[Invite de commandes](#tab/tabid-cmdline)

L’exécution du Q# programme à partir de l’invite de commandes nécessite uniquement une petite modification du Q# fichier.
...
1. Tout d’abord, nous importons le `qsharp` module, qui inscrit le chargeur de module pour l' Q# interopérabilité. 
    Cela permet Q# aux espaces de noms (par exemple `NamespaceQFT` , que nous avons définis dans notre Q# fichier) d’apparaître en tant que modules python, à partir desquels nous pouvons importer des Q# opérations.
2. Ensuite, importez les Q# opérations que nous appellerons directement---dans ce cas, `Perform3qubitQFT` .
    Nous avons uniquement besoin d’importer le point d’entrée dans un Q# programme _not_ (par exemple, les opérations telles que `H` et `R1` , qui sont appelées par d’autres Q# opérations, mais jamais par l’hôte classique).
3. Pour simuler des Q# opérations ou des fonctions, utilisez le formulaire `<Q#callable>.simulate(<args>)` pour les exécuter sur l' `QuantumSimulator()` ordinateur cible. 
...
> En général, Q# les opérations sont indépendantes des ordinateurs sur lesquels elles sont exécutées, mais certaines fonctionnalités telles que `DumpMachine` peuvent se comporter différemment.

4. Lors de l’exécution de la simulation, l’appel d’opération retourne des valeurs telles que définies dans le Q# fichier.
...
En détail pour la première ligne de notre état d’entrée $ \ket {000} $ : _ **`|0>:`** cette ligne correspond à l’état de la `0` base de calcul (étant donné que notre état initial après l’allocation était $ \ket {000} $, nous pensons qu’il s’agit du seul État avec l’amplitude de probabilité à ce stade).
...
Pour implémenter des mesures dans un Q# programme, nous utilisons l' `M` opération (from `Microsoft.Quantum.Intrinsic` ), qui retourne un `Result` type.
...
Pour mieux comprendre le tableau retourné qui sera imprimé dans la console, nous pouvons en ajouter un autre `Message` dans le Q# fichier juste avant l' `return` instruction :
...
## <a name="use-the-no-locq-libraries"></a>Utiliser les Q# bibliothèques
Comme nous l’avons mentionné dans l’introduction, la plupart des Q# reposs de Power dans le fait qu’il vous permet de faire abstraction des soucis liés à la gestion de qubits individuels.
...
Les Q# bibliothèques contiennent l’opération [QFT](xref:Microsoft.Quantum.Canon.QFT) , que vous pouvez simplement prendre et demander pour un nombre quelconque de qubits.
Pour essayer, définissez une nouvelle opération dans votre Q# fichier qui a le même contenu `Perform3QubitQFT` , mais avec tout ce qui est du premier `H` au `SWAP` remplacé par deux lignes simples :
```qsharp
...
Pour avoir accès à ces opérations, ajoutez des `open` instructions pour leurs espaces de noms respectifs au début du Q# fichier :
```qsharp
...
Pour voir l’avantage réel de l’utilisation des opérations de la Q# bibliothèque, modifiez le nombre de qubits à une valeur autre que `3` :
```qsharp
