...
$ $ \begin{align} \ket{\Psi} & = \sum_{x \Dans \\{0, 1\\} ^ n, y \Dans \\{0, 1\\} ^ m} \alpha (x, y) \ket{x} \ket{y} $ $
...
Par exemple, considérez une phase Orcale $O _f $ pour une fonction qubit unique $f $.
Puis, $ $ \begin{align} O_f \ket{+} & = O_f (\ket{0} + \ket{1})/\sqrt{2} \\\\ & = ((-1) ^ {f (0)} \ket{0} + (-1) ^ {f (1)} \ket{1})/\sqrt{2} \\\\ & = (-1) ^ {f ( 0)} (\ket{0} + (-1) ^ {f (1)-f (0)} \ket{1})/\sqrt{2} \\\\ & = (-1) ^ {f (0)} Z ^ {f (0)-f (1)} \ket{+}.
\end{align} $ $
