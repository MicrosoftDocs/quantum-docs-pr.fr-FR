...
Au sein de cette notation, $ \ket{0 } $ ne doit pas faire référence à un État à qubit unique, mais plutôt un *Registre qubit* stockant un encodage binaire de $0 $ .  Les différences entre ces deux notations sont généralement claires du contexte.  Cette Convention est utile pour simplifier le premier exemple qui peut être écrit de l’une des manières suivantes :
...
Voici un autre exemple de la façon dont vous pouvez utiliser la notation Dirac pour décrire un État Quantum, prenez en compte les méthodes équivalentes suivantes pour écrire un État Quantum qui est une superposition égale sur chaque chaîne de longueur possible $n$

$ $ H ^ {\otimes n } \ket{0 } = \frac{1 } {2 ^ {n/2 } } \ sum_ {j = 0 } ^ {2 ^ n-1 } \ket{j } = \ket { +} ^ {\otimes n } .
$$

Vous vous demandez peut-être pourquoi la somme passe de $0 $ à $2 ^ {n } -1 $ pour $n $ bits.  Tout d’abord, Notez qu’il y a des configurations de $2 ^ {n } $ différentes que $n $ bits peut prendre.  Vous pouvez voir cela en notant qu’un bit peut prendre $2 $ valeurs, mais deux bits peuvent prendre $4 $ valeurs, et ainsi de suite. En général, cela signifie qu’il y a au plus 2 ^ n $ chaînes de bits possibles, mais la plus grande valeur encodée dans l’une d’entre elles $1 \cdots 1 = 2 ^ n-1 $ et par conséquent, il s’agit de la limite supérieure de la somme.
En guise de note, dans cet exemple, nous n’avons pas utilisé $ \ket { +} ^ {\otimes n } = \ket { +} $ dans l’analogie avec $ \ket{0 } ^ {\otimes n } = \ket{0 } $, car cette Convention de notation est généralement réservée pour l’état de la base de calcul avec chaque qubit initialisé à zéro.  Bien qu’une telle convention soit rationnelle dans ce cas, elle n’est pas utilisée dans la documentation quantum computing.
...
Par exemple, imaginez que nous souhaitons calculer la probabilité de mesurer l’État $ \ket { \Psi } = \frac{3 } {5 } \ket{1 } + \frac{4 } {5 } \ket{0 } $ à l’aide d’un programme Quantum pour mesurer les États comme étant $ \ket { +} $ ou $ \ket { -} $. La probabilité que l’appareil produise la sortie de l’État est $ \ket { -} $ est 
...
Si, au lieu de cela, nous envisageons de mesurer le premier qubit d’un État $1 à plusieurs qubit $ , nous pouvons également décrire ce processus de manière pratique à l’aide de projecteurs et de la notation Dirac :
...
$ $ P (\text{First qubit = 1 } ) = \Psi ^ \dagger (e \_ {10} e \_ {10 } ^ \dagger + e \_ {11} \_ {11 } ^ \dagger) \Psi = | e \_ {10 } ^ \dagger \Psi | ^ 2 + | e \_ {11 } ^ \dagger \Psi | ^ 2, $ $
