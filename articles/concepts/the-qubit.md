...
Si un bit, ou un chiffre binaire, peut avoir la valeur $0 $ ou $1 $ , un qubit peut avoir une valeur qui est l’une ou l’autre de ces deux ou une superposition quantum de $0 $ et $1 $ .

L’état d’un seul qubit peut être décrit par un vecteur de colonne à deux dimensions de la norme d’unité, autrement dit, l’amplitude du carré de ses entrées doit être cumulée à $1 $ . Ce vecteur, appelé vecteur d’État Quantum, contient toutes les informations nécessaires pour décrire le système Quantum qubit tout comme un bit unique contient toutes les informations nécessaires pour décrire l’état d’une variable binaire.

Tout vecteur de colonne à deux dimensions de nombres réels ou complexes avec la norme $1 $ représente un État Quantum possible détenu par un qubit. Par conséquent, $ \begin{ bmatrix } \alpha \\ \\ \beta \end{ bmatrix } $ représente un État qubit si $ \alpha $ et $ \beta $ sont des nombres complexes qui satisfont à $ | \alpha | ^ 2 + | \beta | ^ 2 = 1 $ . Voici quelques exemples de vecteurs d’État Quantum valides représentant qubits.

$ $ \begin{ bmatrix } 1 \\ \\ 0 \end{ bmatrix } , \begin{ bmatrix } 0 \\ \\ 1 \end{ bmatrix } , \begin{ bmatrix } \frac{1 } {\sqrt{2 } } \\ \\ \frac{1 } {\sqrt{2 } } \end{ bmatrix } , \begin{ bmatrix } \frac{1 } {\sqrt{2 } } \\ \\ \frac { -1 } {\sqrt{2 } } \end{ bmatrix } , \text { et} \begin{\frac{1 bmatrix } } {\sqrt{2 } } \\ \\ \frac{i } {\sqrt{2 } } \end{ bmatrix } . $ $
...
Bien qu’il s’agisse d’un exemple trivial (comme <xref:microsoft.quantum.intrinsic.h>  l’opération est autonome), vous pouvez voir comment cela devient inestimable pour les opérations qubit plus complexes.
...
Nous pouvons classer les portes de Quantum en deux catégories : les portes Clifford et le $ porte-$T.
...
Avec la porte non Clifford (la porte de $T $ ), ces opérations peuvent être composées pour rapprocher toute transformation unitaire sur un qubit unique.
