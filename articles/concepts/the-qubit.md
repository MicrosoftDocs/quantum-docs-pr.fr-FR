...
Bien qu’il s’agisse d’un exemple trivial (étant donné que l’opération de <xref:microsoft.quantum.intrinsic.h> est autonome), vous pouvez voir comment cela devient inestimable pour les opérations qubit plus complexes.
Pour plus d’informations, consultez [opérations et fonctions](xref:microsoft.quantum.techniques.opsandfunctions).
