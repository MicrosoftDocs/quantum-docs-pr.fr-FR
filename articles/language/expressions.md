...
L’équivalence entre les valeurs simples et les tuples à un seul élément décrit dans [le modèle de type](xref:microsoft.quantum.language.type-model#tuple-types) supprime l' `(6)` ambiguïté entre en tant `(6)` que groupe et comme tuple à un seul élément.

## <a name="symbols"></a>Symboles

Le nom d’un symbole lié ou assigné à une valeur de type `'T` est une expression de type `'T`.
Par exemple, si le symbole `count` est lié à la valeur `5`entière, `count` est une expression d’entier.

## <a name="numeric-expressions"></a>Expressions numériques

Les expressions numériques sont des expressions `Int`de `BigInt`type, `Double`ou.
Autrement dit, il s’agit de nombres entiers ou à virgule flottante.

`Int`les littéraux dans Q # sont identiques aux littéraux entiers en C#, sauf qu’aucun « l » ou « L » de fin n’est requis (ou autorisé).
Les entiers hexadécimaux et binaires sont pris en charge avec un préfixe « 0x » et « 0b » respectivement.

`BigInt`les littéraux dans Q # sont identiques aux chaînes d’entiers volumineuses dans .NET, avec un « l » ou « L » de fin.
Les nombres entiers hexadécimaux sont pris en charge avec un préfixe « 0x ».
Par conséquent, Voici toutes les utilisations valides `BigInt` des littéraux :
...
`Double`les littéraux dans Q # sont identiques aux littéraux doubles en C#, sauf qu’aucun « d » ou « D » de fin n’est requis (ou autorisé).

À partir d’une expression de tableau de tout type `Int` d’élément, une expression peut `Length` être formée à l’aide de la fonction intégrée, avec l’expression de `(` tableau `)`placée entre parenthèses, et.
...
Autrement dit, `a % b` aura toujours le même signe que et `a` `b * (a / b) + a % b` sera toujours égal `a`à.
Par exemple :
...
Les deux `Bool` valeurs littérales `true` sont `false`et.

Compte tenu des deux expressions du même type primitif, les `==` opérateurs `!=` binaires et peuvent être utilisés pour construire `Bool` une expression.
...
La comparaison d' `Qubit` égalité pour les valeurs est l’égalité d’identité ; autrement dit, si les deux expressions identifient le même qubit.
L’état des deux qubits n’est pas comparé, accédé, mesuré ou modifié par cette comparaison.

La comparaison d' `Double` égalité des valeurs peut être trompeuse en raison des effets arrondis.
...
À partir de deux expressions booléennes `and` , `or` les opérateurs binaires et peuvent être utilisés pour construire une nouvelle expression booléenne qui a la valeur true si les deux expressions (REEE, ou les deux) sont vraies.

À partir de toute expression booléenne `not` , l’opérateur unaire peut être utilisé pour construire une nouvelle expression booléenne qui a la valeur true si l’expression constitutive est false.

## <a name="string-expressions"></a>Expressions de chaîne

Q # autorise l’utilisation de chaînes dans l' `fail` instruction et la `Log` fonction standard.
...
La syntaxe Q # pour les interpolations de chaînes est un sous-ensemble de la syntaxe C# 7,0 ; Q # ne prend pas en charge les chaînes interpolées textuelles (multiligne).
Consultez [*chaînes interpolées*](https://docs.microsoft.com/dotnet/csharp/language-reference/keywords/interpolated-strings) pour la syntaxe C#.
...
Les seules `Qubit` expressions sont des symboles liés à des `Qubit` valeurs ou à des éléments `Qubit` de tableau de tableaux.
Il n’existe `Qubit` aucun littéral.

## <a name="pauli-expressions"></a>Expressions Pauli

Les quatre `Pauli` valeurs, `PauliI` `PauliX` `PauliY`,, et `PauliZ`, sont toutes des expressions `Pauli` valides.

À part cela, les seules `Pauli` expressions sont des symboles liés à des `Pauli` valeurs ou à des éléments `Pauli` de tableau de tableaux.
...
À part cela, les seules `Result` expressions sont des symboles liés à des `Result` valeurs ou à des éléments `Result` de tableau de tableaux.
En particulier, Notez que `One` n’est pas le même que l' `1`entier et qu’il n’y a pas de conversion directe entre eux.
Il en va de même `Zero` pour `0`et.

## <a name="range-expressions"></a>Expressions de plage

Compte tenu des `Int` trois `start`expressions `step`,, `stop`et `start .. step .. stop` , est une expression de plage dont le `start`premier élément est, `start+step`le deuxième élément est `start+step+step`, le troisième élément est `stop` , etc., jusqu’à ce que soit passé.
Une plage peut être vide si, par exemple, `step` est positif et `stop < start`.
Le dernier élément de la plage `stop` est si la différence entre `start` et `stop` est un multiple entier de ; `step` autrement dit, la plage est incluse aux deux extrémités.

À partir de `Int` deux `start` expressions `stop`quelconques et, `start .. stop` est une expression de plage `start .. 1 .. stop`qui est égale à.
Notez que le implicite `step` est + 1 même `stop` si est inférieur `start`à ; dans ce cas, la plage est vide.
...
Par exemple, `X` est un littéral d’opération qui fait référence à l' `X` opération de bibliothèque `Message` standard, et est un littéral de fonction qui fait `Message` référence à la fonction de bibliothèque standard.

Si une opération prend en `Adjoint` charge functor, `Adjoint op` est une expression d’opération.
De même, si l’opération prend en `Controlled` charge functor, `Controlled op` est une expression d’opération.
Les types de ces expressions sont spécifiés dans les [functors](xref:microsoft.quantum.language.type-model#functors).

Les functors`Adjoint` ( `Controlled`et) lient plus étroitement que tous les autres opérateurs, à l’exception de `!` l’opérateur Unwrap et `[]`de l’indexation de tableau avec.
...
Ainsi, pour appeler le résultat de l' `Builder` appel à partir du paragraphe précédent, la syntaxe correcte est la suivante :

```qsharp
(Builder(3))(2)
```

Lors de l’appel d’un pouvant être appelé par un type paramétrable, les paramètres de type réels peuvent `<` être `>` spécifiés entre crochets pointus et après l’expression pouvant être appelée.
...
Par exemple, si `Func` a une `('T1, 'T2, 'T1) -> 'T2`signature `Op1` , `Op2` et `(Qubit[] => Unit is Adj)`possède la signature `Op3` , et `(Qubit[] => Unit)`possède la signature `Func` , `Op1` pour appeler en tant que `Op2` premier argument, comme deuxième `Op3` argument, et comme troisième :

```qsharp
let combinedOp = Func<(Qubit[] => Unit), (Qubit[] => Unit is Adj)>(Op1, Op2, Op3);
```

La spécification de type est requise `Op3` , `Op1` car et ont des types différents, de sorte que le compilateur traite cette opération comme ambiguë sans la spécification.
...
Dans Q #, un appelable partiellement appliqué est exprimé en écrivant une expression d’appel normale, mais en utilisant un trait `_`de soulignement,, pour les arguments non spécifiés.
...
Par exemple, si `Op` a le `((Int, ((Qubit, Qubit), Double)) => Unit is Adj)`type :

- `Op(5,(_,_))`a le `(((Qubit,Qubit), Double) => Unit is Adj)`type, et ainsi `Op(5,_)`de suite.
...
Par exemple, si `Op` a le `(('T1, Qubit, 'T1) => Unit : Adjoint)`type :
...
Par exemple, si `IntPair` est un type défini par l’utilisateur basé `(Int, Int)`sur, `IntPair(2,3)` est un littéral valide de ce type.
...
Par exemple, si `IntPair` est un type défini par l’utilisateur avec le `(Int, Int)`type sous `s` -jacent et qu’il `IntPair(2,3)`s’agit `s!` d’une `(2,3)`variable avec une valeur, est alors.
...
Ainsi, si `WrappedPair` est un type défini par l’utilisateur avec le `IntPair`type sous `t` -jacent, et est `WrappedPair(IntPair(1,2))`une variable `t!!` de valeur `(1,2)`, est alors.
...
`!`et `[]` liez la position. autrement dit, `a[i]![3]` doit être lu comme `((a[i])!)[3]`: prenez le `i`'th élément de `a`, Désencapsulez-le, puis récupérez le troisième élément de la valeur désencapsulée (qui doit être un tableau).
...
Par exemple, si `Op1`, `Op2`et `Op3` sont tous des `Qubit[] => Unit`, mais `Op1` prend `Adjoint`en `Op2` charge `Controlled`, prend `Op3` en charge et prend en charge les deux :
...
Au lieu `new ★[0]`d’utiliser `★` , où est l’espace réservé pour un type approprié, permet de créer le tableau souhaité de longueur zéro.
...
À partir d’un type `Int` et d’une `new` expression, l’opérateur peut être utilisé pour allouer un nouveau tableau de la taille donnée.
Par exemple, `new Int[i+1]` allouerait un `Int` nouveau tableau `i+1` avec des éléments.
...
À partir d’une expression de `Range` tableau et d’une expression, une nouvelle expression peut `[` être `]` formée à l’aide de l’opérateur de segment de tableau et.
...
Par exemple, si `a` est lié à un tableau de `Double`s, `a[3..-1..0]` est une `Double[]` expression qui contient les quatre premiers éléments de `a` , mais dans l’ordre inverse, tels qu’ils `a`apparaissent dans.
...
Par exemple, si `a` et `b` sont tous deux des tableaux `Int`de s, un segment de la concaténation est exprimé comme suit :
...
À partir d’une expression de `Int` tableau et d’une expression, une nouvelle expression peut `[` être `]` formée à l’aide de l’opérateur d’élément de tableau et.
...
Par exemple, si `a` et `b` sont tous deux des tableaux `Int`de s, un élément de la concaténation est exprimé comme suit :
...
Une expression de copie et de mise à jour est une expression de `expression1 w/ expression2 <- expression3`la forme `expression1` , où doit être de `T[]` type pour un `T`certain type. Le second `expression2` définit les index du ou des éléments à modifier par rapport au tableau dans `expression1` et doit être de type `Int` ou de type. `Range` Si `expression2` est de type `Int`, `expression3` doit être de type `T`. Si `expression2` est de type `Range`, `expression3` doit être de type `T[]`.

Une expression `arr w/ idx <- value` de copie et de mise à jour construit un nouveau tableau avec tous les éléments définis sur l’élément `arr`correspondant dans, à l’exception des éléments situés `idx`à, qui sont définis sur le ou les un ( `value`s) dans. Par exemple, si `arr` contient un tableau `[0,1,2,3]`, 
...
Par exemple, `a==b ? c | d`.
Dans cet exemple, la valeur de l’expression conditionnelle est `c` si `a==b` a la valeur true `d` et si elle est false.
...
Par exemple, dans le cas `a==b ? C(qs) | D(qs)`, si `a==b` a la valeur true `C` , l’opération est appelée, et si elle a la valeur false, seul `D` sera appelé.
...
Functors `Adjoint` et Bind `Controlled` après l’indexation de tableau, mais avant tous les autres opérateurs.
...
---------|----------|---------|---------------
 fin`!` | Unaire | Unwrap (Désencapsuler) | Tout type défini par l’utilisateur
